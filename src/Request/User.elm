module Request.User exposing (edit, login, test, register, storeSession)
...
test : { r | email : String, password : String } -> Http.Request User
test { email, password } =
    let
        user =
            Encode.object
    ...
    in
    Decode.field "user" User.decoder
        |> Http.post (apiUrl "/users/test") body
